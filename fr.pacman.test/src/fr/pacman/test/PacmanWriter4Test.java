...
class PacmanWriter4Test extends AcceleoWorkspaceFileWriter {
	 ...
	 * 
	 * @param p_target        File in which this writer will append text.
	 * @param p_appendMode    Tells us whether the former content of the file should
	 *                        be deleted.
	 * @param p_merge         Tells us whether we should merge the new generated
	 *                        content with the existing file content.
	 * @param p_charset       Encoding that's to be used to create the file with the
	 *                        merged content.
	 * @param p_lineDelimiter le délimiteur de lignes
	 * @throws IOException Thrown if the target cannot be read.
	 */
	public PacmanWriter4Test(final String p_lineDelimiter, final File p_target, final boolean p_appendMode,
	...
	/**
	 * Constructs a writer that will use JMerge to merge the content of the file
	 * existing at path <em>filePath</em> with its new content. Note that the file
	 * will be written with the default System encoding if using this.
	 * 
	 * @param p_target        File in which this writer will append text.
	 * @param p_appendMode    Tells us whether the former content of the file should
	 *                        be deleted.
	...
	public void write(final String p_str, final int p_off, final int p_len) throws IOException {
		...
		if ("\r\n".equalsIgnoreCase(_lineDelimiter)) {
			...
			// Si le délimiteur de ligne a été changé dans les propriétés, il faut changer
			// le code généré.
