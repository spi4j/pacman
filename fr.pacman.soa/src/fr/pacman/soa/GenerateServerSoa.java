...
public class GenerateServerSoa extends PacmanGenerator_Abs {
	 ...
	 */
	public static final String c_MODULE_FILE_NAME = "generateServerSoa";
	 ...
	 */
	public GenerateServerSoa(final EObject p_model, final File p_targetFolder, final List<? extends Object> p_arguments)
			throws IOException {
		super(p_model, p_targetFolder, p_arguments);
		if (p_model instanceof Namespace) {
			_TEMPLATE_NAMES = new String[] { "GenerateServerSystem" };
		} else if (p_model instanceof DTO) {
			_TEMPLATE_NAMES = new String[] { "GenerateServerDto" };
	 ...
	 *                     loaded.
	 */
	public GenerateServerSoa(final URI p_modelURI, final File p_targetFolder, final List<? extends Object> p_arguments)
			throws IOException {
	...
	public static void main(final String[] p_args) {
		try {
			if (p_args.length < 2) {
				...
				final GenerateServerSoa v_generator = new GenerateServerSoa(v_modelURI, v_folder, v_arguments);
				// AcceleoPreferences.switchTraceability(true);
				// final TraceabilityAcceleoGenerator traceabilityGenerator = new
				// TraceabilityAcceleoGenerator(generator,
				// TraceabilityAcceleoGenerator.getLaunchId(args[1], folder.getName(), args[0],
				// folder.getAbsolutePath(), arguments));
