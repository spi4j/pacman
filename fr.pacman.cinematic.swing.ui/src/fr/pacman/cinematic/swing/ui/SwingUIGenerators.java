package fr.pacman.cinematic.swing.ui;
...
import fr.pacman.cinematic.swing.GenerateClientCinematicSwing;
...
/**
 * UI Generator for swing client based on ".cinematic" model file.
 * 
 * @author MINARM
 */
public class SwingUIGenerators extends PacmanUIGenerator_Abs {
	 ...
	 */
	public SwingUIGenerators(final IFile p_selected) {
		super(p_selected);
	}
	...
	protected List<PacmanGenerator_Abs> getGenerators() {
		...
		v_generators.add(new GenerateClientCinematicApi());
		v_generators.add(new GenerateClientCinematicSwing());
		return v_generators;
	}
	...
	protected List<String> getIncompatibleProperties() {
		return Arrays.asList(ProjectProperties.getIsLibraryRs(),
				"SWING".equalsIgnoreCase(ProjectProperties.getClient()) ? "true" : "false");
	}
	...
	protected List<PacmanUIGenerator_Enum> getCompatibleModels() {
		return Arrays.asList(PacmanUIGenerator_Enum.CINEMATIC);
	}
