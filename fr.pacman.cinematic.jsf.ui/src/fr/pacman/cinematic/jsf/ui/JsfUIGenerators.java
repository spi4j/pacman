package fr.pacman.cinematic.jsf.ui;
...
import fr.pacman.cinematic.jsf.GenerateClientCinematicJsf;
...
/**
 * UI Generator for jsf client based on ".cinematic" model file.
 * 
 * @author MINARM
 */
public class JsfUIGenerators extends PacmanUIGenerator_Abs {
	 ...
	 */
	public JsfUIGenerators(final IFile p_selected) {
		super(p_selected);
	}
	...
	protected List<PacmanGenerator_Abs> getGenerators() {
		...
		v_generators.add(new GenerateClientCinematicJsf());
		return v_generators;
	}
	...
	protected List<String> getIncompatibleProperties() {
		return Arrays.asList(ProjectProperties.getIsLibraryRs(),
				"JSF".equalsIgnoreCase(ProjectProperties.getClient()) ? "true" : "false");
	}
