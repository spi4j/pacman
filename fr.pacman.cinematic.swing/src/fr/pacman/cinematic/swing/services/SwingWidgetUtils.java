...
/**
 * Gestion des widgets Swing
 * @author MINARM
 */
public class SwingWidgetUtils extends WidgetUtils_Abs
{
   ...
   public String getImplementationClass (final Widget p_widget)
   {
      try
      {
         ...
         final SwingToolkitWidget_Itf v_widget = getWidget(p_widget);
         if (v_widget != null)
         {
         ...
         else
         {
            ...
            {
               throw new IncompatibleWidgetException("Implémentation non définie pour le widget " + p_widget.getName());
            }
...
public String getImplementationPackageJavaService(final Widget p_widget){return getImplementationPackage(p_widget);}
    ...
    */
   private SwingToolkitWidget_Itf getWidget (final Widget p_widget) throws IncompatibleWidgetException
   {
      ...
      if (StandardToolkitWidget_Itf.c_toolkitName.equals(v_toolkit.getName()))
      {
         ...
         return findWidget(p_widget, StandardSwingToolkit_Enum.values());
      }
      // utilisation du toolkit Swing
      else if (SwingToolkit_Enum.c_toolkitName.equals(v_toolkit.getName()))
      {
         // on récupère l'énumération des informations liées à ce widget
         return findWidget(p_widget, SwingToolkit_Enum.values());
      }
      else
      {
         return null;
