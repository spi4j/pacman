 ...
 * 
 * @TODO : modifications a effectuer. Attention aux valeurs nulles on simplifie
 *       pour l'instant mais voir si la propriete n'est pas remontee.
 */
public abstract class SafranGenerator_Abs<Type> {
	 ...
	 * Constructor.
	 * 
	 * @param p_input     The input of generation (URI or EObject)
	 * @param p_arguments are the other arguments
	 */
	...
	/**
	 * Launches the generation. For the properties manager, the exit procedure is
	 * only for writing the properties files in case of change and not for closing
	 * any resources, so if an exception is thrown during he generation, it is not
	 * important (no finally).
	 * 
	 * @param p_monitor This will be used to display progress information to the
	 *                  user.
	 ...
	 * 
	 * @param p_input     l'entree de la generation
	 * @param p_arguments les arguments
	 * @return la liste des generateurs a appeler
	 * @throws IOException erreur lors de l'instanciation d'un generateur
	 */
	protected abstract List<PacmanGenerator_Abs> getPacmanGenerators(Type p_input, List<? extends Object> p_arguments)
			throws IOException;

	/**
	 * Retourne la liste des projets a rafraichir une fois les generations
	 * effectuees.
	 * 
	 * @return le nom des projets a rafraichir apres generation
	 */
	protected abstract List<String> getProjectsNamesToRefresh();
	 ...
	 */
	protected abstract List<Boolean> getValuesOfIncompatibleProperties();

	/**
	 * Retourne la liste des fichiers de modelisation COMPATIBLES pour lancer le
	 * generateur.
	 * 
	 * @return l'extension necessaire sous forme d'enumeration (ex : .entity, .soa,
	 *         etc...)
	 */
	protected abstract List<SafranGenerator_Enum> getValuesOfCompatibleModels();

	/**
	 * Retourne un indicateur permettant de savoir si le générateur doit lancer la
	 * séquence d'organisation des imports automatisée par Eclipse.
	 * 
	 * @return 'true' si le générateur est éligible à l'organisation automatique des
	 *         imports, sinon 'false'.
	 */
	 ...
	 * @param p_type le type du parent recherche
	 * @param <T>    le type du parent recherche
	 * @return le parent trouve avec le type p_type, ou null si aucun parent de ce
	 *         type n'a ete trouve
	 */
	...
	/**
	 * @return le projet contenant le pom.xml principal (lu dans le fichier
	 *         properties)
	 */
	...
	/**
	 * @return le nom du projet qui contient les modeles (lu dans le fichier
	 *         properties).
	 */
