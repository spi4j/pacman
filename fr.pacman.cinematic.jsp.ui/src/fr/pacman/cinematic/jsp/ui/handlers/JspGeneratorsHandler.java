package fr.pacman.cinematic.jsp.ui.handlers;
...
import fr.pacman.cinematic.jsp.ui.JspUIGenerators;

/**
 * UI handler for jsp client generator. Generic algorithm for the
 * method but for pacman generators, only unique selection is allowed.
 * 
 * @author MINARM
 */
public class JspGeneratorsHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent p_event) throws ExecutionException {
		...
		while (v_iterator.hasNext()) {
			...
			if (v_selected instanceof IFile) {
				new JspUIGenerators((IFile) v_selected).generate();
			}
		}
		return null;
	}
