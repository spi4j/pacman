package fr.pacman.cinematic.jsp.ui;
...
import fr.pacman.cinematic.jsp.GenerateClientCinematicJsp;
...
/**
 * UI Generator for jsp client based on ".cinematic" model file.
 * 
 * @author MINARM
 */
public class JspUIGenerators extends PacmanUIGenerator_Abs {
	 ...
	 */
	public JspUIGenerators(final IFile p_selected) {
		super(p_selected);
	}
	...
	protected List<PacmanGenerator_Abs> getGenerators() {
		...
		v_generators.add(new GenerateClientCinematicJsp());
		return v_generators;
	}
	...
	protected List<String> getIncompatibleProperties() {
		return Arrays.asList(ProjectProperties.getIsLibraryRs(),
				"JSP".equalsIgnoreCase(ProjectProperties.getClient()) ? "true" : "false");
	}
